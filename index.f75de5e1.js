...
function createDataCells(person) {
    ...
    headCells.forEach((td)=>{
        ...
        if (key === "age") data.textContent = person.died - person.born;
        else if (key === "century") data.textContent = Math.ceil(person.died / 100);
        else if (key === "gender") data.textContent = person.sex === "m" ? "Male" : "Female";
        else data.textContent = person[key];
        cells.push(data);
    });
    return cells;
}
